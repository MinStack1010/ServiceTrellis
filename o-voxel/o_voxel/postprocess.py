...
def to_fbx(
    ...
    """
    Convert an extracted mesh to a format ready for FBX export.
    ...
    if use_tqdm:
        pbar = tqdm(total=6, desc="Extracting for FBX")
    if verbose:
    ...
    diffuse_image = Image.fromarray(np.concatenate([base_color, alpha], axis=-1))
    metallic_image = Image.fromarray(np.repeat(metallic[..., None], 3, axis=-1))
    roughness_image = Image.fromarray(np.repeat(roughness[..., None], 3, axis=-1))
    ...
    material.kwargs['metallic_texture'] = metallic_image
    material.kwargs['roughness_texture'] = roughness_image
